...
* [Quickstart](#quickstart)
  ...
  * [2. Configure your `MODULE.bazel` to use rules_swift_package_manager.](#2-configure-your-modulebazel-to-use-rules_swift_package_manager)
    * [Use `{module}_swift_package` repository for updating packages](#use-module_swift_package-repository-for-updating-packages)
    * [(Optional) Enable `{module}_swift_deps_info` generation for the Gazelle plugin](#optional-enable-module_swift_deps_info-generation-for-the-gazelle-plugin)
...
```python
bazel_dep(name = "rules_swift_package_manager", version = "1.11.3")
```
<!-- END MODULE SNIPPET -->
...
use_repo(
    swift_deps,
    "{module}_swift_deps_info",  # This is generated by the ruleset.
...
#### Use `{module}_swift_package` repository for updating packages

The `swift_deps` module extension will by default generate a `{module}_swift_package` repository which can be used to execute `swift package` commands.
...
You must use the `{module}_swift_package` repository to update or resolve package versions and update the `Package.resolved`. In scenarios where multiple Bazel modules use **rules_swift_package_manager**, the `Package.swift` files are "merged" and then resolved, as such, direct usage of `swift package` is not possible.

#### (Optional) Enable `{module}_swift_deps_info` generation for the Gazelle plugin

If you will be using the [Gazelle plugin for Swift], you will need to enable the generation of
the `{module}_swift_deps_info` repository by enabling `declare_swift_deps_info`.
