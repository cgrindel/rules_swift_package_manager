...
* [Quickstart](#quickstart)
  ...
  * [4. Generate the SPM cache.](#4-generate-the-spm-cache)
  * [5. Run `bazel mod tidy`.](#5-run-bazel-mod-tidy)
  * [6. Build and test your project.](#6-build-and-test-your-project)
  * [7. Check in `Package.swift`, `Package.resolved`, `swift_deps_cache/`, and `MODULE.bazel`.](#7-check-in-packageswift-packageresolved-swift_deps_cache-and-modulebazel)
...
```python
bazel_dep(name = "rules_swift_package_manager", version = "1.17.1")
```
...
use_repo(
    ...
    # The name of the Swift package repositories will be added to this declaration in step 5 after
...
### 4. Generate the SPM cache.

Run the cache utility to generate the cached SPM dump and description JSON files using the
Bazel-configured Swift toolchain. This avoids the loading-phase toolchain mismatch described in
[Caching Swift Package Manifests](/docs/swift_package_caching.md), and bootstraps `Package.resolved`
on its first run if you don't have one yet.

```sh
bazel run @swift_package//:cache -- --mode update --output_dir swift_deps_cache
```

The utility writes a `swift_deps_cache/` directory at the workspace root and updates the
`dump_manifests` and `desc_manifests` dicts on the `swift_deps.from_package` tag in your
`MODULE.bazel`. Re-run this command whenever you change `Package.swift` or bump your Swift
toolchain.
...
### 7. Check in `Package.swift`, `Package.resolved`, `swift_deps_cache/`, and `MODULE.bazel`.

- The `Package.swift` file is used by `rules_swift_package_manager` to generate information about
  your project's dependencies.
- The `Package.resolved` file specifies the exact versions of the downloaded dependencies.
- The `swift_deps_cache/` directory holds the cached SPM dump and description JSON files used at
  load time so they always match the Bazel-resolved Swift toolchain.
- The `MODULE.bazel` contains the declarations for your external dependencies, including the
  `dump_manifests` and `desc_manifests` dicts wired up by the cache utility.
