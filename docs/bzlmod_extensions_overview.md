...
| <a id="swift_deps.configure_package-publicly_expose_all_targets"></a>publicly_expose_all_targets |  Allows to expose internal build files required for package compilation. The structure and labels of exposed build files may change in future releases without requiring a major version bump.   | Boolean | optional |  `False`  |
