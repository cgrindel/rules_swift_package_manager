...
def _registry_swift_package_impl(repository_ctx):
    ...
    repository_ctx.report_progress("Downloading package metadata for {id}@{version}".format(id = id, version = version))
    ...
    repository_ctx.report_progress("Downloading source archive for {id}@{version}".format(id = id, version = version))
    ...
    repository_ctx.report_progress("Generating Bazel build files for {id}@{version}".format(id = id, version = version))
