...
def _download_archive(*, checksum, id, output, repository_ctx, registry_url, version):
    ...
    if find_prefix_result.return_code != 0:
        fail("Failed to list contents of the downloaded archive from: {}".format(archive_url))
    possible_prefixes = find_prefix_result.stdout.strip().split("\n")
    if not possible_prefixes:
        fail("No contents were found in the downloaded archive from: {}".format(archive_url))
    if len(possible_prefixes) > 1:
        fail("Expected a single prefix for the source archive, found: {}".format(possible_prefixes))
    archive_directory = "{}/{}".format(download_output, possible_prefixes[0])

    # Move the contents of the temporary location to the requested output, we use a cp and rm instead
    # of mv to ensure only the contents are moved (and not the parent directory).
    cp_result = repository_ctx.execute(["cp", "-a", "{}/.".format(archive_directory), output])
    if cp_result.return_code != 0:
        fail("Failed to copy the contents of the downloaded archive from: {} to: {}".format(
            archive_directory,
            output,
        ))
...
def _get_resolved_pin_for_url(*, registry_url, repository_ctx, resolved_pkg_map, url):
    """Returns the resolved pin for the given URL by querying the registry and matching on the resolved package map.

    This registry endpoint is provided when determining a package's registry identity given only
    ...
    curl_args = [
        ...
        "-s",
        "-H",
        "\"Accept: {accept_header}\"".format(accept_header = _REGISTRY_JSON_ACCEPT_HEADER["Accept"]),
        "-G",
        "-d",
    ...
    # TODO: is this what SPM does? for now, pick the first matching identity from the resolved package map.
    resolved_pin = None
...
def _replace_scm_identities_in_pkg_info(*, pkg_info, registries, repository_ctx, resolved_pkg_map):
    """Replaces the SCM identity of the dependencies in provided package info and resolved package map."""
    ...
    for dep in pkg_info.dependencies:
        ...
        if not dep.source_control.pin or not dep.source_control.pin.location:
            ...
            print("Unable to find URL for {identity}, cannot replace SCM identity".format(identity = dep.identity))
            replaced_dependencies.append(dep)
            continue

        # Find first identifier from any of the registries which matches the resolved identity,
        # preferring scoped registries first.
        ...
        if not resolved_pin:
            # buildifier: disable=print
            print("Unable to find resolved pin for {identity}, cannot replace SCM identity".format(
...
def _registry_swift_package_impl(repository_ctx):
    ...
    pkg_ctx = pkg_ctxs.read(repository_ctx, directory, env, resolved_pkg_map = resolved_pkg_map)
